...
df = pd.read_csv("AVINData.csv")

ADHD = df[df["DX"]==1]
Normal = df[df["DX"]==0]

features_name = list(df.columns[1:-5])
...
for count, feature in enumerate(features_name):
    ...
    ax.set(ylabel=None)
    plt.savefig(f"/home/zaman/Desktop/thesis new documents/AVIN/MI/Modeling_Features/features{count}.png",
